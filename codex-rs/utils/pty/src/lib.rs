...
// Android's Bionic libc exposes `posix_openpt`/`grantpt`/`unlockpt`/`ptsname_r`,
// but not `openpty`. The `portable_pty` crate uses `openpty` on Unix, so we
// provide a small `openpty` shim for Android to keep Termux builds working.
...
pub unsafe extern "C" fn openpty(
    ...
    // `portable_pty` always passes valid pointers here.
    ...
    if !name.is_null() {
        // Caller provides a sufficiently-sized buffer (openpty contract).
